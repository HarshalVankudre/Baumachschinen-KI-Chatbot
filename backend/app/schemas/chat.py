...
class MessageCreate(BaseModel):
    """Message creation request schema with hybrid search controls."""
    message: str = Field(max_length=2000, description="User message (max 2000 characters)")
    conversation_id: Optional[str] = Field(default=None, description="Conversation ID for state tracking (optional)")
    ...
    alpha: Optional[float] = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Hybrid search balance: 0.0=keyword only (BM25), 1.0=vector only, 0.75=default (75% vector + 25% keyword)"
    )
    enable_rerank: Optional[bool] = Field(
        default=None,
        description="Enable Cohere reranking for improved relevance (None=use system default)"
    )
...
class StreamSourceEvent(BaseModel):
    ...
    source_type: Literal['pinecone', 'weaviate', 'model_knowledge'] = Field(description="Source type")
    source_name: Optional[str] = Field(default=None, description="Source name (document, etc.)")
