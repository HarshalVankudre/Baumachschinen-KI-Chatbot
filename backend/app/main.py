...
async def recover_orphaned_processes() -> None:
    """
    ...
    Raises:
        No exceptions - errors are logged and suppressed to prevent startup failure
    """
    ...
    try:
        ...
        cutoff_time = datetime.now(UTC) - timedelta(minutes=ORPHANED_JOB_TIMEOUT_MINUTES)
        ...
        logger.error(f"Failed to recover orphaned processes: {str(e)}", exc_info=True)
...
# Proxy headers middleware - trust X-Forwarded-* headers from Cloudflare
