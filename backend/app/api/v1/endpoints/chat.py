...
async def send_message_streaming(
    ...
    2. Retrieves context from Weaviate using hybrid search (vector + BM25)
    3. Optionally applies Cohere reranking for improved relevance
    4. Streams AI response token-by-token using Server-Sent Events
    5. Saves AI response to conversation
    6. Returns source citations and metadata

    Hybrid Search Parameters:
    - alpha: Balance between vector (semantic) and keyword (BM25) search
      * 0.0 = Pure keyword search (BM25)
      * 1.0 = Pure vector search (semantic)
      * 0.75 = Default (75% vector + 25% keyword)
    - enable_rerank: Use Cohere reranking for better results (default: system setting)
    ...
    async def stream_response():
        ...
        try:
            ...
            state_conversation_id = message_data.conversation_id or conversation_id

            # Don't need to classify here - generate_response_stream handles it internally
            # We'll use "technical" as default for token counting (slightly overestimate is fine)
            category = "technical"  # Default assumption for token counting

            # DIAGNOSTIC: Log search parameters for debugging
            logger.info(
                f"[DIAGNOSTIC] Sending message to AI agent - "
                f"conv_id={conversation_id}, "
                f"alpha={message_data.alpha}, "
                f"enable_rerank={message_data.enable_rerank}, "
                f"conversation_history_length={len(conversation_history)}, "
                f"query_preview='{message_data.message[:50]}...'"
            )

            # Step 5: Stream AI response with hybrid search and optional reranking
            # Agent will use Weaviate hybrid search with configurable parameters
            async for token in ai_agent.generate_response_stream(
                query=message_data.message,
                conversation_id=state_conversation_id,  # Pass conversation_id for state tracking
                authorization_level=user.authorization_level,
                conversation_history=conversation_history,
                category=None,  # Let generate_response_stream determine this
                alpha=message_data.alpha,  # Hybrid search balance (vector vs keyword)
                enable_rerank=message_data.enable_rerank,  # Cohere reranking
            ...
            ai_msg = MessageModel(
                ...
                metadata={
                    ...
                    "hybrid_search_alpha": message_data.alpha,
                    "reranking_enabled": message_data.enable_rerank,
                }
            )
...
# Advanced RAG and Feedback endpoints removed in simplification
