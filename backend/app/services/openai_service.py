...
class OpenAIService:
    ...
    async def generate_chat_completion(
        ...
        Args:
            ...
            model: Override default chat model (e.g., "gpt-4o-mini" for cost-effective tasks)
    ...
    async def describe_image_bytes(
        ...
        """
        Call GPT-4 Vision with inline PNG bytes.

        Args:
            image_bytes: Raw image bytes (PNG preferred)
            instruction: Text instruction for the user turn
            max_tokens: Optional max tokens override
            model: Vision-capable model (defaults to gpt-4o)
                ...
                messages=[
                    {
                        ...
                        or "Du bist ein präziser OCR-Analyst für technische Dokumente.",
                    },
                    {
                        "role": "user",
                        "content": [
    ...
    async def describe_image_file(
        ...
        """
        Convenience wrapper that loads an image from disk before describing it.
        """
        data = Path(image_path).read_bytes()
        return await self.describe_image_bytes(
            image_bytes=data,
