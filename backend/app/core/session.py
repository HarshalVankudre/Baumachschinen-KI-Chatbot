...
async def create_session(
    ...
    try:
        ...
        # Only set samesite in production - development needs None for cross-port
...
async def validate_session(request: Request) -> Optional[str]:
    ...
    try:
        # Get cookie
        logger.info(f"[DEBUG] All cookies: {request.cookies}")
        logger.info(f"[DEBUG] Looking for cookie: {settings.session_cookie_name}")
        signed_token = request.cookies.get(settings.session_cookie_name)
        if not signed_token:
            logger.warning(f"[DEBUG] No session cookie '{settings.session_cookie_name}' found. Available cookies: {list(request.cookies.keys())}")
            return None
        logger.info(f"[DEBUG] Found session cookie: {signed_token[:20]}...")
