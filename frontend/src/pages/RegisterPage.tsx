...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Pass password twice for backend compatibility
      await register(email, username, password, password);
      toast({
        title: 'Erfolgreich',
        description: 'Konto erstellt. Bitte überprüfen Sie Ihre E-Mail zur Verifizierung.',
      });
      ...
      if (status === 409) {
        ...
        if (message?.toLowerCase().includes('username')) {
          setErrors({ username: 'Benutzername bereits vergeben' });
        } else if (message?.toLowerCase().includes('email')) {
          setErrors({ email: 'E-Mail bereits registriert' });
        } else {
          setErrors({ general: message || 'Benutzername oder E-Mail bereits vergeben' });
        }
      } else if (status === 400) {
        // Validation errors from backend
        setErrors({ general: message || 'Ungültige Eingaben. Bitte überprüfen Sie Ihre Daten.' });
      } else if (status === 500) {
        setErrors({ general: 'Serverfehler. Bitte versuchen Sie es später erneut.' });
      } else {
        setErrors({ general: message || 'Ein Fehler ist aufgetreten' });
      }
    }
  };
