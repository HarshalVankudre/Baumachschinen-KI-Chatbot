...
function SortableConversationItem({
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        'group relative rounded-xl p-3.5 cursor-pointer transition-all duration-200',
        'hover:bg-muted/50 border-2 hover:shadow-premium-sm',
        isActive
          ? 'bg-primary/10 border-primary shadow-premium-sm'
          : 'bg-background border-transparent',
        isDragging && 'z-50 shadow-premium-lg'
      )}
      onClick={onSelect}
      onDoubleClick={(e) => {
        e.stopPropagation();
...
export function ConversationSidebar({
  ...
  return (
    <div className="w-80 border-r border-border/50 bg-gradient-subtle flex flex-col h-full">
      <div className="p-5 border-b border-border/50 space-y-4">
        <Button
          onClick={onNewConversation}
          className="w-full shadow-premium-md hover:shadow-premium-lg relative"
          variant="accent"
          disabled={loading}
          title="Neue Konversation erstellen (Strg+N)"
        >
          <Plus className="h-4 w-4 mr-2" />
          Neue Konversation
          <kbd className="absolute right-3 hidden md:inline-block px-2 py-0.5 text-xs font-mono bg-white/20 rounded">Strg+N</kbd>
        </Button>
      ...
      <ScrollArea className="flex-1 scroll-smooth scrollbar-premium">
        {filteredConversations.length === 0 ? (
          ...
          >
            ...
            <DragOverlay>
              {activeId ? (
                <div className="rounded-xl p-3.5 bg-background border-2 border-primary shadow-premium-lg opacity-95">
                  <div className="flex items-start gap-2">
