...
export const documentService = {
  ...
  /**
   * Subscribe to real-time document processing updates via SSE
  ...
  subscribeToDocumentUpdates(
    ...
    const url = `${baseURL}/api/documents/stream/${documentId}`;
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.type === 'done' || data.processing_status === 'completed' || data.processing_status === 'failed') {
          eventSource.close();
        }
      } catch (error) {
